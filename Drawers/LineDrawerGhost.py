...
def notXorColor(col1, col2):
    return ((255 - (col1[0]^col2[0])), (255 - (col1[1]^col2[1])), (255 - (col1[2]^col2[2])))
...
def ghostLine(x0, y0, x1, y1, screen, color):
    steep = abs(y1 - y0) > abs(x1 - x0)
    if steep:
        tmp = x0
        x0 = y0
    ...
    for x in range (x0, x1+1):
        if steep: 
            oldColor = screen.get_at((y,x))
            screen.set_at([y, x], notXorColor(oldColor, color))
        else: 
            oldColor = screen.get_at((x,y))
            screen.set_at([x,y], notXorColor(oldColor, color))
        error = error - dy
        if error < 0:
            y = y + ystep
            error = error + dx   
...
while 1:
    ...
    for event in events:
        ...
        if event.type == KEYDOWN:
            if event.key == K_m:
                ...
                flag = not flag
            if (event.key == K_c):
                ghostLine(x0,y0, x1, y1, screen, colors[numcolor])    
    ...
    if pygame.mouse.get_pressed()[0]:
        ...
        x1 = pygame.mouse.get_pos()[0]
        y1 = pygame.mouse.get_pos()[1]
        if (x0 !=  x1 or y0 != y1) and (x1 != oldx1 or y1 != oldy1):
            ghostLine(x0,y0, x1, y1, screen, colors[numcolor])
            ghostLine(x0,y0, oldx1, oldy1, screen, colors[numcolor])            
        oldx1 = x1
        oldy1 = y1
    pygame.display.flip()        
